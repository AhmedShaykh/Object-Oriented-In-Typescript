...
;
const show = new Data("Andrew Tate");
...
console.log(pointer.x, pointer.y);
class ReadData {
    name = "AHM X"; // Initial Value Is No Change
}
;
const read = new ReadData();
console.log(read.name);
class ReadValue {
    value; // Value Assign In One Time No Change Value
    constructor(value) {
        this.value = value;
    }
}
;
const readVal = new ReadValue("Yarn Dev");
console.log(readVal.value);
const readVal2 = new ReadValue("Yarn Start");
...
;
const myCar1 = new Bugatti("Black", 5400000);
myCar1.display();
...
;
const myCar2 = new Tesla("Grey", "AI Drive");
myCar2.drive();
myCar2.color = "Red";
myCar2.drive();
...
classDay.onClass();
const myClass = new School("Ahmed", "Qasim", "6 PM");
...
;
const driving = new ElectricCar(); // This Is Polymorphism
driving.drive();
...
greet.greeting();
class Greeter2 {
    _name; // Private Property Only Use In Own Class 
    constructor(_name) {
        this._name = _name;
    }
    greeting() {
...
greet2.greeting();
class Greeter3 {
    _name = "Metaverse"; // Private Property Not Read Directly & Change It...
    greeting() {
        console.log("Welcome To " + this._name);
    }
}
;
...
console.log(checker["_x"]); // The Problem In TypeScript Is Private Property Assign In Array String!
class Check {
    #x; // JavaScript This New Native Build-In Private Property Feature Solve The Issue!
    constructor(_x) {
        this.#x = _x;
    }
    greeting() {
...
MyStatic.printX(); // In Static Values (Method or Prop) Call No Need Object Create
